...
import type { User, PaginatedResponse, UserFilters, SortDirection, CreateUserRequest, UpdateUserRequest } from '~/types/user';
...
export const useUsersStore = defineStore('users', () => {
  ...
  async function createUser(userData: CreateUserRequest) {
    ...
    try {
      ...
      if (pagination.value.page === 1) {
        users.value = [newUser, ...users.value].slice(0, pagination.value.pageSize);
      }
  ...
  async function deleteUser(id: string) {
    ...
    try {
      ...
      const result = await $fetch<{ success: boolean; message: string }>(`/api/users/${id}`, {
        method: 'DELETE'
      });
